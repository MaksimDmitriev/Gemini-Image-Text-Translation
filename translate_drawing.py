#!/usr/bin/env python3
"""Translate Russian labels in a technical drawing with one Gemini API call."""
...
def background_color(image, box):
    """Estimate the paper colour from a narrow ring around the text."""
    x1, y1, x2, y2 = box
    margin = max(2, round(min(image.size) * 0.003))
    outer = image.crop(
        (max(0, x1 - margin), max(0, y1 - margin),
         min(image.width, x2 + margin), min(image.height, y2 + margin))
    )
    # The median is resistant to dark letters and drawing lines on light paper.
    return tuple(round(value) for value in ImageStat.Stat(outer).median)
...
def translate_image(input_path, output_path, model):
    ...
    print(f"Translated {len(regions)} text regions -> {output_path}")
